...
class CephSalt(Task):
    ...
    def _ceph_salt_bootstrap(self):
        ...
        for host, _ in self.remote_lookup_table.items():
            ...
            self.master_remote.sh("sudo ceph-salt config /ceph_cluster/roles/cephadm add {}"
                                 .format(host))
            self.master_remote.sh("sudo ceph-salt config /ceph_cluster/roles/admin add {}"
                                 .format(host))
        if len(self.remote_lookup_table.keys()) <= 3:
            self.master_remote.sh("sudo ceph-salt config /cephadm_bootstrap/ceph_conf add global")
            self.master_remote.sh("sudo ceph-salt config /cephadm_bootstrap/ceph_conf/global set"
        ...
        self.master_remote.sh("sudo ceph-salt config /containers/registries add "
                              "  prefix=registry.suse.de"
                              " location={} insecure=true".format(registry_cache))
        ...
        self.master_remote.sh("sudo ceph-salt config /cephadm_bootstrap/advanced set fsid {}"
                              .format(fsid))
        self.master_remote.sh("sudo ceph-salt config /cephadm_bootstrap/dashboard/username set "
                              "admin")
        self.master_remote.sh("sudo ceph-salt config /cephadm_bootstrap/dashboard/password set "
                              "admin")
        self.master_remote.sh("sudo ceph-salt config ls")
        if self.ceph_salt_deploy:
            self.master_remote.sh("sudo stdbuf -o0 ceph-salt -ldebug apply --non-interactive")
