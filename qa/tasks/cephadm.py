...
def ceph_deploy_from_spec(ctx, config):
    ...
    #rgw_spec(ctx, config, daemons)
    #mds_spec(ctx, config, daemons)
    #igw_spec(ctx, config, daemons)
    #monitoring_spec(ctx, config, 'prometheus', daemons)
    #monitoring_spec(ctx, config, 'node-exporter', daemons)
    #monitoring_spec(ctx, config, 'alertmanager', daemons)
    #monitoring_spec(ctx, config, 'grafana', daemons)
...
def mon_or_mgr_rolespec(daemon_type, ctx, config):
    ...
    with open( daemon_type + '.yaml', 'w+') as file:
        yaml.safe_dump_all(spec_yaml, file, default_flow_style=False)

    #Deploy daemons from spec file
    _shell(ctx, cluster_name, ctx.ceph[cluster_name].bootstrap_remote,
           ['ceph', 'orch', 'apply', '-i', daemon_type + '.yaml'])
...
def osd_rolespec(ctx, config):
    ...
    for remote, roles in ctx.cluster.remotes.items():
        ...
        num_osds = 0
        temp_spec['service_id'] = 'osd_spec_' + remote.shortname
        temp_spec['placement'] = {'host_pattern': remote.shortname}
        for osd in [r for r in roles
                    if teuthology.is_type('osd', cluster_name)(r)]:
            num_osds += 1
        temp_spec['data_devices'] = {'limit': num_osds}
        osd_spec.append(temp_spec)

    #Create service spec file
    with open('osd.yaml', 'w+') as file:
        yaml.safe_dump_all(osd_spec, file, default_flow_style=False)
...
def rgw_rolespec(ctx, config):
    ...
    for realmzone, nodelist in nodes.items():
        ...
        rgw_spec.append(temp_spec)
    
    if nodes:
        #Create service spec file
        with open('rgw.yaml', 'w+') as file:
            yaml.safe_dump_all(rgw_spec, file, default_flow_style=False)
...
def monitoring_rolespec(daemon_type, ctx, config):
    daemons[daemon_type] = []
    monitoring_spec = {'service_type': daemon_type, 'placement': {'hosts': []}}
    nodes = []
    cluster_name = config['cluster']
    testdir = teuthology.get_testdir(ctx)
    for remote, roles in ctx.cluster.remotes.items():
        for role in [r for r in roles
                    if teuthology.is_type(daemon_type, cluster_name)(r)]:
            monitoring_spec['placement']['hosts'].append(remote.shortname)
            nodes.append(remote.shortname + '=' + role)

    #Create service spec file
    with open(daemon_type + '.yaml', 'w+') as file:
        yaml.safe_dump(monitoring_spec, file, default_flow_style=False)
...
def igw_rolespec(ctx, config):
    ...
    cluster_name = config['cluster']
    poolname = 'iscsi'
    igw_spec = {'service_type': 'iscsi', 'service_id': 'igw', 'placement': {'hosts': []}}
    testdir = teuthology.get_testdir(ctx)
    ...
    for remote, roles in ctx.cluster.remotes.items():
        for role in [r for r in roles
            ...
            c_, _, id_ = teuthology.split_role(role)
            log.info('Adding %s on %s' % (role, remote.shortname))
            igw_spec['placement']['hosts'].append(remote.shortname)
            nodes.append(remote.shortname + '=' + id_)
    if nodes:
        # ceph osd pool create iscsi 3 3 replicated
        _shell(ctx, cluster_name, remote, [
    ...
    #Create service spec file
    with open('igw.yaml'), 'w+' as file:
        yaml.safe_dump_all(igw_spec, file, default_flow_style=False)

    #Deploy daemons from spec file
    _shell(ctx, cluster_name, ctx.ceph[cluster_name].bootstrap_remote,
           ['ceph', 'orch', 'apply', '-i', 'igw.yaml'])
    # ISSUE: HOW TO FIGURE OUT THE DAEMON ID TO REGISTER IT
